...
class StateStringUtils:
    ...
    def state_to_string(self, state):
        ...
        if state.terminate == 1:
            ...
            return 'C(%i,%i,%i,%i,%i,%i)' % (state.filter_depth, state.filter_size, state.stride, state.padding, state.act, state.bias)
        elif state.layer_type == 'pool':
            return 'P(%i,%i,%i)' % (state.filter_size, state.stride, state.padding)
        elif state.layer_type == 'fc':
            return 'FC(%i,%i,%i)' % (state.fc_size, state.act, state.bias)
        return None
    ...
    def convert_model_string_to_states(self, parsed_list, start_state=None):
        ...
        for layer in parsed_list:
            if layer[0] == 'conv':
                if first_layer == 1:
                    first_layer = 0
                    input_ = np.random.normal(127, 60, (self.batchsize, self.input_image_size, self.input_image_size, self.input_image_channels)).astype(float)
                    tf_placeholder = tf.placeholder(tf.float32, shape=[None, self.input_image_size, self.input_image_size, self.input_image_channels], name="inputs")
                    op = tf.layers.conv2d(tf_placeholder, filters=layer[1], kernel_size=[layer[2], layer[2]], strides=(layer[3], layer[3]), 
                        padding=('SAME' if layer[4] ==1 else 'VALID'), activation=eval(activation_list[layer[5]]), 
                        use_bias=layer[6], name='convolution_%d'%(layer_count))
                else:
            ...
            #                             terminate=0))
                ...
                if first_layer == 1:
                    ...
                    input_ = np.random.normal(127, 60, (self.batchsize, self.input_image_size, self.input_image_size, self.input_image_channels)).astype(float)
                    tf_placeholder = tf.placeholder(tf.float32, shape=[None, self.input_image_size, self.input_image_size, self.input_image_channels], name="inputs")
                    op = tf.layers.max_pooling2d(tf_placeholder, pool_size=(layer[1], layer[1]), strides=(layer[2], layer[2]), 
                        padding=('SAME' if layer[3]==1 else 'VALID'), name = 'pooling_%d'%(layer_count))
                else:
                ...
                if first_layer == 1:
                    ...
                    input_ = np.random.normal(127, 60, (self.batchsize, self.input_image_size*self.input_image_size*self.input_image_channels)).astype(float)
                    tf_placeholder = tf.placeholder(tf.float32, shape=[None, self.input_image_size*self.input_image_size*self.input_image_channels], name="inputs")
                    op = tf.layers.dense(inputs=tf_placeholder, units=layer[1], kernel_initializer=tf.ones_initializer(), 
                        activation=eval(activation_list[layer[2]]), use_bias=layer[3], name = 'dense_%d'%(layer_count))
                else:
        ...
        result_dict = {
            ...
            'input_image_channels' : self.input_image_channels,
