...
class Cnn(runtime.Parser):
    ...
    def layers(self, _parent=None):
        ...
        _token = self._peek('CONV', 'INPUT', 'OUTPUT', 'NIN', 'GLOBALAVE', 'BATCHNORM', 'POOL', 'SPLIT', 'FC', 'DROP', 'SOFTMAX', context=_context)
        if _token == 'CONV':
            conv = self.conv(_context)
            return conv
        elif _token == 'INPUT':
            iinput = self.iinput(_context)
            return iinput
        elif _token == 'OUTPUT':
            ooutput = self.ooutput(_context)
            return ooutput
        elif _token == 'NIN':
    ...
    def ooutput(self, _parent=None):
        _context = self.Context(_parent, self._scanner, 'output', [])
        OUTPUT = self._scan('OUTPUT', context=_context)
        result = ['output']
        numlist = self.numlist(_context)
        return result + numlist
